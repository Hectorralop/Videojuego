...
public class Player extends MovingObject{
	...
	public void update(float dt) 
	{
		...
		if(KeyBoard.SHOOT &&  fireRate > fireSpeed && !spawning)
		{
			...
			if(doubleGunOn) {
				...
				Laser l = new Laser(leftGun, heading, Constants.LASER_VEL, angle, Assets.redLaser, gameState);
				Laser r = new Laser(rightGun, heading, Constants.LASER_VEL, angle, Assets.redLaser, gameState);
	...
	public void draw(Graphics g) {
		...
		AffineTransform at1 = AffineTransform.getTranslateInstance(position.getX() + width/2 + 18,
				position.getY() + height/2 + 31);
		
		AffineTransform at2 = AffineTransform.getTranslateInstance(position.getX() + 21,
				position.getY() + height/2 + 31);
		
		at1.rotate(angle, -18, -31);
		at2.rotate(angle, width/2 -21, -31);
